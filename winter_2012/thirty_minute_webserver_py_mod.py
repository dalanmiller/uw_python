...
import os, socket, sys, re, subprocess, tracebrack
...
def list_directory(uri):
    entries = [x for x in os.listdir('.' + uri) if re.search('.py$', x)]
    entries.sort()
...
def get_content(uri):
    ...
    try:
        ...
        if os.path.isfile(path) and re.search('.py$',path):
            call = ['python',path]
            if re.search("socket", open(path).read()):
                b = "That python file was a little too complicated for this exercise!\n\n\n"
            else:
                try:
                    b = subprocess.check_output(call)
                except as e:
                    b = traceback.print_exc(e)
